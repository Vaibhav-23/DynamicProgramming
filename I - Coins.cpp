...
template<typename T>
void getInput(vector<T> &arr){
    for(ll i=0;i<arr.size();i++){
        cin>>arr[i];
    }
}
//debuging part
void debug(vector<double> &arr){
    for(auto &it : arr){
        cout<<it<<" ";
    }
    cout<<"\n";
...
double helper(ll idx,ll cnt,vector<double> &a, vector<vector<double>> &dp){
    if(idx==a.size()){
        ll tails=(idx-cnt);
        if(cnt>tails) return 1.00;
        return 0.00;
    }
    if(dp[idx][cnt]!=-1) return dp[idx][cnt];
    double ans=helper(idx+1,cnt+1,a,dp)*a[idx];
    ans=ans+(helper(idx+1,cnt,a,dp))*(1-a[idx]);
...
void solve(){
    ...
    cout<<fixed<<setprecision(10)<<helper(0,0,a,dp)<<endl;
}
