...
ll helper(ll l,ll r,v1d &a,v2d &dp){
    //base case here
    if(l==r){
        return 0;
    }
    if(dp[l][r]!=-1) return dp[l][r];
    return dp[l][r]=a[r]-a[l]+min(helper(l+1,r,a,dp),helper(l,r-1,a,dp));
}
void solve(){
    ll n;
    cin>>n;
    ...
    sort(all(a));
    v2d dp(n,v1d(n,-1));
    //here dp[l][r] represents minimum value of dispecenraries we can get from the subarray l...r 
    //if array from l...r then obviously we would encounter a situation of a[r]-a[l] which is the maximum sum either and also for the 1st element it would be 0
    cout<<helper(0,n-1,a,dp)<<endl;
}
