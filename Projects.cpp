...
static bool comp(pair<pair<ll,ll>,ll> a,pair<pair<ll,ll>,ll> b){
    if(a.first.second!=b.first.second) return b.first.second>a.first.second;
...
void solve(){
    ...
    ll best=0;
    for(ll i=0;i<n;i++){
        ll start=a[i].first.first,end=a[i].first.second,val=a[i].second;
        auto it = dp.lower_bound(start);
        it--;
        ll curr = it->second+val;
        best=max(curr,best);
        dp[end]=best;
    }
    print(best);
