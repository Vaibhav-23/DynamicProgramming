...
void solve(){
    ...
    vi a(n);
    getInput(a);
    vector<vector<ll>> dp(n,vector<ll>(k+1,0)),pref(n,vector<ll>(k+1,0));
    vi pre(k+1,0);
    for(ll i=0;i<=k;i++){
        if(i<=a[0]){
            dp[0][i]=1;
        }
        if(i>0) pref[0][i]=pref[0][i-1]+dp[0][i];
        else pref[0][i]=1;
    }
    //debug(pref[0]);
    for(ll i=1;i<n;i++){
        for(ll j=0;j<=k;j++){
            if(j-a[i]>0) dp[i][j]=add(pref[i-1][j],-pref[i-1][max(j-a[i]-1,0LL)]);
            else dp[i][j]=pref[i-1][j];
            if(j>0) pref[i][j]=add(pref[i][j-1],dp[i][j]);
            else pref[i][j]=1;
            //cout<<dp[i][j]<<" ";
        }
        //cout<<endl;
