...
void solve(){
    ...
    vector<vector<ll>> dp(n+1,vector<ll>(maxi+1,1e9+1));
    //dp(i,sum) minimum weight req to get sum==sum from 1 to i index
    dp[0][0]=0;
    for(ll i=1;i<=n;i++){
        for(ll sum=0;sum<=maxi;sum++){
            ll nottake=dp[i-1][sum];
            ll take=1e9;
            if(sum>=val[i]) take=dp[i-1][sum-val[i]]+weight[i];
