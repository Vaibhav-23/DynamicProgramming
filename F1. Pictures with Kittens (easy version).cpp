...
void solve(){
    ...
    // debug(a);
    v2d dp(n,vi(x,-1e12));
    //dp[i][j] is we have come till index i and have considered exactly j pics
    //for that dp[i][j] to get the maximum just check last k elements as for every element we are checking 
    // base case dp[0][0]=0;
    dp[0][0]=0;
