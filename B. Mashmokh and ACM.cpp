...
void solve(){
    ...
    //here the state is we have come till ith index
    //and from 0 to i-1 is already filled with last divisior
    //and standing at ith index we are having the current one as last
    for(ll i=1;i<k;i++){
        for(ll last=1;last<=n;last++){
            for(ll j=last;j<=n;j+=last){
