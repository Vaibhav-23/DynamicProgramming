...
void solve(){
    ll a,b;
    cin>>a>>b;
    //dp[i][j] represents the minimum number of cuts required to make rectangle
    //of dimensions i*j to squares
    //Here we can cut horizontal or vertical

    v2d dp(a+1,vi(b+1,1e9));
    for(ll i=1;i<=a;i++){
        for(ll j=1;j<=b;j++){
            if(i==j){
                dp[i][j]=0;
                ...
                //horizontal cut or cutting along the length
                for(ll k=1;k<i;k++){
                    dp[i][j]=min(dp[i][j],1+dp[k][j]+dp[i-k][j]);
                }
                //vertical cut or cutting along the height
                for(ll k=1;k<j;k++){
