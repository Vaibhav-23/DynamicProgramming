...
void solve(){
    ...
    // debug(a);
    v2d dp(n+1,v1d(h,-1e9));
    dp[0][0]=0;
    //writing 1st index as base case
    // if(a[1]>=l && a[1]<=r){
    //     dp[1][a[1]]=1;
    // }
    // else dp[1][a[1]]=0;
    ...
    //state-> here the state is we are at ith index and slept at jth hour the maximum possible good subsequences till the ith position if we are sleeping at jth hour

    //if j is in the range of l,r then we can consider this also as a good subsequence
    
    for(ll i=1;i<=n;i++){
        for(ll j=0;j<h;j++){
            dp[i][j]=max(dp[i-1][(j-a[i]+h)%h],dp[i-1][(j-a[i]+h+1)%h]);
